...
import asyncio
import aiohttp
from pywikibot.comms.aiosseclient import aiosseclient
...
class AsyncEventStreams(EventStreams):

    def __init__(self, **kwargs):
        super(AsyncEventStreams, self).__init__(**kwargs)
        loop = asyncio.get_event_loop()
        loop.run_until_complete(self._get_event())

    async def agen(self):
        async for event in aiosseclient(self.url):
            yield event
